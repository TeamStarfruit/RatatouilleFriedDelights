...
//            return;
        ...
        RegistrySetBuilder worldgenBuilder = new RegistrySetBuilder()
                ...
                .add(ForgeRegistries.Keys.BIOME_MODIFIERS,     RFDBiomeModifiers::bootstrap);
        ...
        generator.addProvider(event.includeServer(), new RFDStandardRecipeGen(output));
        generator.addProvider(event.includeServer(), new RFDSequencedAssemblyRecipeGen(output));
        generator.addProvider(event.includeServer(), new RFDGlobalLootModifierProvider(output));


        if (event.includeServer()) {
            RFDRecipeProvider.registerAllProcessing(generator, output);
        }
    }
}
